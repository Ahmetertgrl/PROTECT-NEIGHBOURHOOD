...
// Magnum tabancasýnýn tüm sistemýni yöneten sýnýf
public class Magnum : MonoBehaviour
{
    ...
    bool zoomvarmi;                        // Oyuncunun su an zoom yapýp yapmadýðýný tutar
    public GameObject CrossHair;           // Ekrandaki crosshair nesnesi
    ...
    public ParticleSystem Mermiizi;        // Yuzeyde býrakýlan mermi izi efekti

    [Header("DÝÐERLERÝ")]
    public Camera benimcamim;             // Oyuncu kamerasý (Raycast ve zoom için)
    float FieldCamPos;                    // Kameranýn baþlangýç Field of View deðeri
    float YaklasmaPov = 45;              // Zoom yapýldýðýnda kullanýlacak FOV deðeri
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Mouse0) && !Input.GetKey(KeyCode.Mouse1))
        {
            ...
            {
                if (!GameKontrolcu.OyunDurdumu) // Oyun duraklatýlmamýssa ateþ et
                {
    ...
    {
        AtesEtmeteknikÝslemleri(yakinlasmavarmi); // Kovan, ses, efekt, animasyon iþlemleri
        RaycastHit hit;
        ...
        if (Physics.Raycast(benimcamim.transform.position, benimcamim.transform.forward, out hit, menzil))
        {
            ...
            if (hit.transform.gameObject.CompareTag("Dusman"))
            {
                ...
                hit.transform.gameObject.GetComponent<Dusman>().Darbeal(DarbeGucu);  // Düþmana hasar ver
            }
    ...
    // Ateþ etmenin kovan, ses, efekt ve animasyon iþlemlerini yapar
    ...
    void MermiKaydet(string silahturu, int mermisayisi)
    {
        ...
        switch (silahturu)
        {
            ...
            case "Sniper":
                ...
                break;
            case "Magnum":
                // Magnum mermisi alýndýysa bu silahýn toplam sayýsýný artýr ve UI'ý güncelle
